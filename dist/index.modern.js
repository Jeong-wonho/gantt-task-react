    ...
    if (i + 1 !== dates.length && date.getTime() < now.getTime() && dates[i + 1].getTime() >= now.getTime() || i !== 0 && i + 1 === dates.length && date.getTime() < now.getTime() && addToDate(date, date.getTime() - dates[i - 1].getTime(), "millisecond").getTime() >= now.getTime()) {
      if (now.toDateString() === date.toDateString()) {
        ...
        if (currentHour < 6) {
          ...
          relativePosition = columnWidth * (1 / 3);
        } else if (currentHour >= 12 && currentHour < 18) {
          relativePosition = columnWidth * (2 / 3);
        } else if (currentHour >= 18 && currentHour <= 24) {
          relativePosition = columnWidth * (2.5 / 3);
        }
...
var Gantt = function Gantt(_ref) {
  var tasks = _ref.tasks,
      ...
      onExpanderClick = _ref.onExpanderClick,
      onScrollToToday = _ref.onScrollToToday,
      isTodayChecked = _ref.isTodayChecked;
  ...
  useEffect(function () {
    if (isTodayChecked && todayRef.current && onScrollToToday) {
      var _todayRef$current;

      var todayX = (_todayRef$current = todayRef.current) === null || _todayRef$current === void 0 ? void 0 : _todayRef$current.x.baseVal.value;
      onScrollToToday();
      handleScrollToToday(todayX / 1.3);
    }
  }, [onScrollToToday, isTodayChecked]);

  var handleScrollToToday = function handleScrollToToday(todayX) {
    var newScrollX = todayX;
  ...
  var loadMoreDates = function loadMoreDates() {
    var lastDate = dateSetup.dates[dateSetup.dates.length - 1];
    var newDates = seedDates(lastDate, new Date(lastDate.getTime() + 10 * 24 * 60 * 60 * 1000), viewMode);
    var uniqueNewDates = newDates.filter(function (date) {
      return !dateSetup.dates.some(function (existingDate) {
        return existingDate.getTime() === date.getTime();
      });
    });
    console.log('uniqueNewDates', uniqueNewDates);
    setDateSetup(function (prevSetup) {
      return _extends({}, prevSetup, {
        dates: [].concat(prevSetup.dates, uniqueNewDates)
      });
    });
